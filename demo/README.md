...
`ROOT="$(git rev-parse --show-toplevel)" && git --git-dir="$ROOT/fixtures/repo.git" --work-tree="$ROOT" diff --no-ext-diff --cached HEAD -- state.txt && git --git-dir="$ROOT/fixtures/repo.git" --work-tree="$ROOT" diff --no-ext-diff -- state.txt`.
